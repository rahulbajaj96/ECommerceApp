...
export async function getColorParamsFromID(colors_array, color_selected) {
    console.log('color selected', color_selected);
    ...
    let color_selected_params = '';
    for (let i = 0; i < colors_array.length; i++) {
        if (colors_array[i].id == color_selected) {
            color_selected_params = colors_array[i]
            break;
        }
    }
...
export async function getSizeParamsFromName(sizes_array, size_selected) {
...
export async function getSizeParamsFromID(sizes_array, size_selected) {
    console.log('size_selected', size_selected);
    ...
    let size_selected_params = '';
    for (let i = 0; i < sizes_array.length; i++) {
        if (sizes_array[i].id == size_selected) {
            size_selected_params = sizes_array[i]
            break;
        }
    }
...
export function getCategoryParamsFromID(category_array, category_selected) {
    let category_selected_params = '';
    for (let i = 0; i < category_array.length; i++) {
        if (category_array[i].id == category_selected) {
            category_selected_params = category_array[i];
            break;
        }
    }
